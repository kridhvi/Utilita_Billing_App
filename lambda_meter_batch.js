...
class SmartMeterBilling {
  ...
  processUsageDataBatch(usageDataBatch) {
    for (const usageData of usageDataBatch) {
...
exports.handler = async (event) => {
  try {
    const { peakHoursRate, offPeakHoursRate, usageDataBatch } = event;

    const smartMeterBilling = new SmartMeterBilling(peakHoursRate, offPeakHoursRate);

    smartMeterBilling.processUsageDataBatch(usageDataBatch);
